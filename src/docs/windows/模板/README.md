...
`isAdmin` (uin int64, groupCode ...int64) bool
...
注：群主人可以用`/grant -r GroupAdmin 群号`添加。

支持两个参数，第一个参数为QQ号，第二个参数为群号（可选），请求参数的格式为string，返回值为bool格式。

当只输入QQ号时，仅判定其是否为主人；附加群号后，会在判断主人的基础上判断其是否为群主人。

···
{{ $uin := index .at_targets 0 }}
{{ $ret := isAdmin $uin .group_code }}
{{ if $ret }}
是主人或群主人
{{ else }}
不是主人或群主人
...
分开两次进行调用，一次仅输入QQ号，一次附带群号，便可以单独判断是主人还是群主人。
```
...
`getTime` ( "now" | "2024-06-22 15:15:47" | time string, "datetime" | "dateonly" | "timeonly" | "stamp" | format string) string
...
```
手动指定时间 不入參会报错 入參格式(2022-12-31 23:59:51)
...
`getTimeStamp` ( "2024-06-22 15:15:47" | time sring) int64
...
`getUnixTime` ( 1719040547 | Stamp int64, "datetime" | "dateonly" | "timeonly" | "stamp" | format string ) string

```
时间戳转日期：
{{ getUnixTime $n "dateonly" }}
时间戳转时间：
{{ getUnixTime $n "timeonly" }}
时间戳转完整时间：
{{ getUnixTime $n "datetime" }}
```
...
`delStrSlice` (arr []string, sub string) []string
...
`readLine` (path string, line int64) string, err0r
...
`writeLine` (path string, line int64, content string) err0r
...
`addScore` (uin int64, groupCode int64, num int64) 
...
`subScore` (uin int64, groupCode int64, num int64)
...
`setScore` (uin int64, groupCode int64, num int64) 
...
`getScore` (uin int64, groupCode int64)
